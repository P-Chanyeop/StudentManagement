...
- **출석부 조회**
  - ✅ 오늘 날짜 출석 조회 API 존재
  - ⚠️ "출석한 순서대로, 하원하는 순서대로" 정렬 - AttendanceRepository 쿼리 수정 필요
  - ⚠️ 출석/미출석 분리 조회 기능 추가 필요
  - ⚠️ 체크박스 (수업 완료 여부) UI는 프론트엔드 미구현
...
- **청구서 카톡/문자 발송**
  - 🔴 청구서 생성 기능 미구현
  - ⚠️ 기본 문자 발송 인프라는 존재 (MessageService)
  - 🔴 나이스페이먼츠 연동 미구현
...
### ⚠️ 연동 필요
- **SMS API 연동**
  - ⚠️ 실제 SMS API 미연동 (TODO 주석 존재)
  - 테스트 모드로 자동 발송 완료 처리 중
  - 발신번호 사전 등록 필요
...
- **레벨테스트 관리**
  ...
  - ✅ 결과 및 피드백 기록
  - ✅ 자동 문자 알림
  - ⚠️ 캘린더 UI는 프론트엔드 미구현 (백엔드 API 완성)
  - ⚠️ 캘린더 기능 추가 시 +400,000원
...
11. 상담 이력 관리
12. 상담 이력 Excel 내보내기
13. 파일 업로드/다운로드 (녹음 파일)
14. 공지사항 관리
15. 자동 문자 발송 (지각, 수강권 만료, 레벨테스트)
16. 공휴일 관리
17. 마이페이지 API
18. JWT 인증 및 권한 관리
19. 보안 강화 (XSS, Path Traversal 등)
20. 메시지 발송 이력 관리
...
1. 🔴 청구서 발송 시스템
2. 🔴 나이스페이먼츠 연동
3. 🔴 레벨테스트 캘린더 UI (+400,000원)
4. 🔴 보호자 권한 세분화 (+100,000원)
...
| 레벨테스트 캘린더 | +400,000원 | 🔴 미구현 |
| 보호자 권한 세분화 | +100,000원 | 🔴 미구현 |
...
현재 백엔드 핵심 기능은 거의 완성되었으며 (75% 완료), **프론트엔드 개발이 가장 중요한 다음 단계**입니다.
...
2. SMS API 업체 선택 (예: 알리고, 문자나라 등)
3. 네이버 예약 크롤링 필요 여부 재확인
4. 레벨테스트 캘린더 추가 여부 (+400,000원)
5. 청구서 발송 기능 필요 여부 및 일정
